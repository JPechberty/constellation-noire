...
('FLAG 5.5: Élévation Privilèges', 'NIGHTFIRE{privilege_escalation_master}', 10, 'Control', 
 'Élever vos privilèges à root sur la Control Station via exploitation sudo.', 0, FALSE, TRUE),
...
('FLAG 7.5: Port Knocking', 'NIGHTFIRE{secret_knock_revealed}', 10, 'Satellite', 
 'Réussir la séquence de port knocking pour accéder au satellite ARGOS-7B.', 0, FALSE, TRUE),
...
-- FLAG 5.5: Élévation Privilèges
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(6, 'Sur Control Station, l''utilisateur ctrlops n''a pas accès direct à /root/. Vérifiez vos permissions sudo avec: sudo -l', 5, 1),
(6, 'Le binaire vim est autorisé en sudo sans mot de passe. Consultez GTFOBins pour voir comment exploiter vim.', 3, 2),
(6, 'Utilisez vim pour obtenir un shell root: sudo vim, puis dans vim tapez :set shell=/bin/bash puis :shell', 2, 3);

-- FLAG 6: Exfiltration GPS
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(7, 'La base de données télémétrie est accessible depuis la Control Station.', 5, 1),
(7, 'mysql -h telemetry-db -u telemetry -pT3l3m3try2024! argos_telemetry', 3, 2),
(7, 'Regardez la table target_coordinates pour les coordonnées GPS.', 2, 3);

-- FLAG 7: Accès Satellite
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(8, 'Les credentials du satellite sont dans la documentation technique ou dans les notes système de Control Station.', 5, 1),
(8, 'Le satellite écoute sur le port SSH custom 8976, IP: 172.24.0.10', 3, 2),
(8, 'Mais attention: le port semble fermé. Consultez les notes sur Control Station pour la procédure d''accès.', 2, 3);

-- FLAG 7.5: Port Knocking
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(9, 'Le port 8976 du satellite est fermé par défaut. Cherchez dans les notes de Control Station des procédures d''accès spéciales.', 5, 1),
(9, 'Le satellite utilise le port knocking. La séquence se trouve dans ~/notes/satellite_access.txt sur Control Station.', 3, 2),
(9, 'Séquence de knock: 7777 → 8888 → 9999. Utilisez la commande knock depuis Control Station: knock 172.24.0.10 7777 8888 9999', 2, 3);

-- FLAG 8: Exploitation Ada
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(10, 'Les codes sources Ada sont dans /opt/satellite_control/ sur le satellite.', 5, 1),
(10, 'attitude_control.adb a un buffer overflow, propulsion_system.adb a un integer overflow.', 3, 2),
(10, 'Essayez d''envoyer une commande de plus de 256 caractères ou une durée négative/très grande.', 2, 3);

-- FLAG 9: Sabotage
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(11, 'Lisez le fichier README_SATELLITE.txt pour les scénarios de sabotage possibles.', 5, 1),
(11, 'Vous pouvez épuiser le carburant avec EMERGENCY_BRAKE répété ou exploiter les vulnérabilités Ada.', 3, 2);

-- FLAG 10: Nettoyage
INSERT INTO hints (flag_id, content, penalty_points, order_num) VALUES
(12, 'Les logs système sont généralement dans /var/log/', 5, 1),
(12, 'Effacez au moins: logs SSH (auth.log), logs système (syslog), et logs applicatifs.', 3, 2);
...
('total_flags', '12', 'Nombre total de flags standards (10 principaux + 2 bonus intermédiaires)'),
